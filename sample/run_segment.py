...
def run_segment(
    ...
    if input_yomi_type != "phoneme":
        conv_funtion = (
            sp_inserter.conv2julius
            if not like_openjtalk
            else sp_inserter.conv2openjtalk
        )
    ...
    if not only_2nd_path:
        ...
        raw_first_output = sp_inserter.julius_sp_insert(
            str(wav_file), f"/tmp/first_pass_{utt_id}", hmm_model
        )
    ...
    raw_second_output = sp_inserter.julius_phone_alignment(
        str(wav_file), f"/tmp/second_pass_{utt_id}", hmm_model
    )
...
def main():
    ...
    parser.add_argument(
        "--hmm_model", help="support mono-phone model only", required=True
    )
