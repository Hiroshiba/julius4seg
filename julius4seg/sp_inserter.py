...
def julius_sp_insert(
    target_wav_file: str, aliment_file_signiture: str, model_path: str
) -> [str]:
    julius_args = {
        "-h": model_path,
        "-input": "file",
...
def get_sp_inserted_text(raw_output: str, debug_symbol="") -> (str, [int]):
    ...
    return (
        re.sub("sp_[\d+]", "<sp>", matched.group(1)),
...
def get_sp_inserterd_phone_seqence(raw_output: str, debug_symbol="") -> str:
    try:
        pass1_best_phonemeseq = next(
            s for s in raw_output if s.startswith("pass1_best_phonemeseq")
        )
...
def julius_phone_alignment(
    target_wav_file: str, aliment_file_signiture: str, model_path: str = None
) -> [str]:
    julius_args = {
        "-h": model_path,
        "-palign": "",
...
def get_time_alimented_list(raw_output: str) -> [str]:
    ...
    return [
        ...
        for s in map(lambda x: r.search(x), raw_output)
        if s is not None
    ]


def frame_to_second(time_list):
    return [
        (
